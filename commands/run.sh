...
container_name="$(docker_compose_project_name)_${run_service}_build_${BUILDSCALE_BUILD_NUMBER}"
override_file="docker-compose.buildscale-${BUILDSCALE_BUILD_NUMBER}-override.yml"
...
if [[ "$(plugin_read_config RUN_LABELS "true")" =~ ^(true|on|1)$ ]]; then
  ...
  run_params+=(
    "--label" "com.buildscale.pipeline_name=${BUILDSCALE_PIPELINE_NAME}"
    "--label" "com.buildscale.pipeline_slug=${BUILDSCALE_PIPELINE_SLUG}"
    "--label" "com.buildscale.build_number=${BUILDSCALE_BUILD_NUMBER}"
    "--label" "com.buildscale.job_id=${BUILDSCALE_JOB_ID}"
    "--label" "com.buildscale.job_label=${BUILDSCALE_LABEL}"
    "--label" "com.buildscale.step_key=${BUILDSCALE_STEP_KEY}"
    "--label" "com.buildscale.agent_name=${BUILDSCALE_AGENT_NAME}"
    "--label" "com.buildscale.agent_id=${BUILDSCALE_AGENT_ID}"
  )
fi
...
if [[ "$(plugin_read_config PROPAGATE_ENVIRONMENT "false")" =~ ^(true|on|1)$ ]] ; then
  if [[ -n "${BUILDSCALE_ENV_FILE:-}" ]] ; then
    ...
    while read -r var; do
      run_params+=("-e" "${var%%=*}")
    done < "${BUILDSCALE_ENV_FILE}"
  else
    echo -n "🚨 Not propagating environment variables to container as \$BUILDSCALE_ENV_FILE is not set"
  fi
fi
...
# Parse BUILDSCALE_DOCKER_DEFAULT_VOLUMES delimited by semi-colons, normalized to
# ignore spaces and leading or trailing semi-colons
IFS=';' read -r -a default_volumes <<< "${BUILDSCALE_DOCKER_DEFAULT_VOLUMES:-}"
...
if [[ -n "${BUILDSCALE_REPO_MIRROR:-}" ]]; then
  run_params+=("-v" "$BUILDSCALE_REPO_MIRROR:$BUILDSCALE_REPO_MIRROR:ro")
fi
...
if [[ ! "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_MOUNT_SSH_AGENT:-false}" = 'false' ]] ; then
  ...
  if [[ "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_MOUNT_SSH_AGENT:-''}" =~ ^(true|on|1)$ ]]; then
    MOUNT_PATH=/root
  else
    MOUNT_PATH="${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_MOUNT_SSH_AGENT}"
  fi
...
# Optionally handle the mount-buildscale-agent option
if [[ "$(plugin_read_config MOUNT_BUILDSCALE_AGENT "false")" == "true" ]]; then
  if [[ -z "${BUILDSCALE_AGENT_BINARY_PATH:-}" ]] ; then
    if ! command -v buildscale-agent >/dev/null 2>&1 ; then
      echo -n "+++ 🚨 Failed to find buildscale-agent in PATH to mount into container, "
      echo "you can disable this behaviour with 'mount-buildscale-agent:false'"
    else
      BUILDSCALE_AGENT_BINARY_PATH=$(command -v buildscale-agent)
    fi
  fi
fi

# Mount buildscale-agent if we have a path for it
if [[ -n "${BUILDSCALE_AGENT_BINARY_PATH:-}" ]] ; then
  run_params+=(
    "-e" "BUILDSCALE_JOB_ID"
    "-e" "BUILDSCALE_BUILD_ID"
    "-e" "BUILDSCALE_AGENT_ACCESS_TOKEN"
    "-v" "$BUILDSCALE_AGENT_BINARY_PATH:/usr/bin/buildscale-agent"
  )
  if [[ -n "${BUILDSCALE_AGENT_JOB_API_SOCKET:-}" ]] ; then
    run_params+=(
      "-e" "BUILDSCALE_AGENT_JOB_API_SOCKET"
      "-e" "BUILDSCALE_AGENT_JOB_API_TOKEN"
      "-v" "$BUILDSCALE_AGENT_JOB_API_SOCKET:$BUILDSCALE_AGENT_JOB_API_SOCKET"
    )
  fi
fi
...
if [[ ! -f "$override_file" ]] ; then
  ...
  if [[ "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_REQUIRE_PREBUILD:-}" =~ ^(true|on|1)$ ]]; then
...
if [[ $dependency_exitcode -ne 0 ]] ; then
  ...
  if [[ -n "${BUILDSCALE_AGENT_ACCESS_TOKEN:-}" ]] ; then
...
if [[ -n "${BUILDSCALE_COMMAND}" ]]; then
  shell_disabled=''
fi

# Handle shell being disabled
if [[ "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_SHELL:-}" =~ ^(false|off|0)$ ]] ; then
  shell_disabled=1

# Show a helpful error message if a string version of shell is used
elif [[ -n "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_SHELL:-}" ]] ; then
...
elif plugin_read_list_into_result BUILDSCALE_PLUGIN_DOCKER_COMPOSE_SHELL ; then
  shell_disabled=''
...
if [[ -n "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_COMMAND:-}" ]] ; then
  echo "🚨 The Docker Compose Plugin’s command configuration option must be an array."
  exit 1
fi

# Parse plugin command if provided
if plugin_read_list_into_result BUILDSCALE_PLUGIN_DOCKER_COMPOSE_COMMAND ; then
  for arg in "${result[@]}" ; do
    command+=("$arg")
  done
fi

if [[ ${#command[@]} -gt 0 ]] && [[ -n "${BUILDSCALE_COMMAND}" ]] ; then
...
if [[ -n "${BUILDSCALE_COMMAND}" ]] ; then
  if [[ $(echo "$BUILDSCALE_COMMAND" | wc -l) -gt 1 ]]; then
  ...
  run_params+=("${BUILDSCALE_COMMAND}")
  display_command+=("'${BUILDSCALE_COMMAND}'")
...
if [[ "${BUILDSCALE_PLUGIN_DOCKER_COMPOSE_COLLAPSE_LOGS:-false}" = "true" ]]; then
  group_type="---"
else
  group_type="+++"
fi
